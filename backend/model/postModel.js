...
exports.getPosts = async (requestData, response) => {
    ...
    let whereClause = 'post_table.deleted_at IS NULL';
    const params = [];

    if (query) {
        whereClause += ` AND (post_table.post_title LIKE ? OR post_table.post_content LIKE ?)`;
        const keywordPattern = `%${query.trim()}%`;
    ...
    const results = await dbConnect.query(sql, params, response);
